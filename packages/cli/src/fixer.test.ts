  ...
  it("works", () => withTemp(async (tempdir: string) => {
    ...
    for (const filepath of files1) {
      ...
      if (stat1.isFile() && stat2.isFile()) {
        ...
        if (content1.length !== content2.length || !content1.every((b, i) => b === content2[i])) {
          unmatchedFiles.push(filepath);
        }
    ...
    if (unmatchedFiles.length > 0) {
      ...
      if (process.env['UPDATE_SNAPSHOTS']) {
        ...
        const outputSaveDir = await fs.promises.mkdtemp(path.resolve(__dirname, "./__fixtures__/simple-project/output-"));
...
async function withTemp<T>(cb: (tempdir: string) => Promise<T>): Promise<T> {
  const tempdir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "hi18n-test-"));
  let result: T;
  try {
