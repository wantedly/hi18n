...
export function create(context: Rule.RuleContext): Rule.RuleListener {
  ...
  tracker.listen("book", (node, _captured) => {
    ...
    if (missingIdsSet.size > 0) {
      ...
      missingIds.sort();
      context.report({
        node: objinfo.body as (Rule.Node | TSInterfaceBody | TSTypeLiteral) as Rule.Node,
        messageId: "missing-translation-ids",
        *fix(fixer) {
          const candidates = collectCandidates(context.getSourceCode(), objinfo.signatures);
          ...
          for (const missingId of missingIds) {
            ...
            if (candidateIndex !== undefined) {
              ...
              if (insertAt === 0) {
                ...
                const text = `\n${" ".repeat(indent)}${JSON.stringify(missingId)}: Message;`;
                const token = context.getSourceCode().getFirstToken(objinfo.body as (Rule.Node | TSInterfaceBody | TSTypeLiteral) as Rule.Node)!;
                ...
                const text = `\n${" ".repeat(indent)}${JSON.stringify(missingId)}: Message;`;
                const node = extendNode(context.getSourceCode(), lastCandidate.node ? lastCandidate.node as (Node | TSPropertySignature) as Node : lastCandidate.commentedOut[lastCandidate.commentedOut.length - 1]!);
...
const RE_LIKELY_COMMENT_START = /^\s*("[^"\\]*"|'[^'\\]*'|\w+)\s*:\s*(?:Message|$)/;
const RE_LIKELY_COMMENT_END = /(?:Message|>)\s*[,;]\s*(?:\/\/.*(?:\n|$)|\/\*(?:[^*]|\*(?!\/))\*\/\s*)*$/;

function collectCandidates(sourceCode: SourceCode, signatures: TSSignature[]): Candidate[] {
  const candidateNodes: Candidate[] = [];
  for (const signature of signatures) {
    const precedingComments = getPrecedingComments(sourceCode, signature as (Node | TSSignature) as Node);
  ...
  {
    const lastComments = getLastComments(sourceCode, signatures as (Node | TSSignature)[] as Node[]);
...
function getLastComments(sourceCode: SourceCode, nodes: Node[]): Comment[] {
  ...
  const maybePunct = sourceCode.getTokenAfter(lastNode, { includeComments: false });
  let lastToken: Node | Comment | AST.Token =
    maybePunct && maybePunct.type === "Punctuator" && (maybePunct.value === "," || maybePunct.value === ";") ?
      maybePunct : lastNode;
