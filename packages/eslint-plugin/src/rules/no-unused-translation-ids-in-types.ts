...
import { NewExpressionExt, TSInterfaceBody, TSPropertySignature, TSSignature, TSTypeLiteral } from "../estree-ts";
import { extractAsObjectType, findTypeParameter } from "./no-nonstandard-catalogs";
      ...
      if (!usedIdsSet.has(key)) {
        context.report({
          ...
          *fix(fixer) {
            ...
            const text = context.getSourceCode().getText(signature as (Node | TSPropertySignature) as Node);
            yield fixer.replaceText(signature as (Node | TSPropertySignature) as Node, commentOut(text, indent));
          },
        });
      }
    }
